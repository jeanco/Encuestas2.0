...
//	echo "preferencias";
	...
	$objPHPExcel->getProperties()->setCreator("Servicios Mexicanos de Ingenieria Civil")
	    ...
	    $prog= 1;
		foreach ($client->excelUsuario($consulta,$skip,$limit) as $fila) 
		 {			
	...
	header('Content-Disposition: attachment;filename="'.$_GET['capturista'].'-PD.xlsx"');
	...
	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	header('Content-Disposition: attachment;filename="'.$_GET['capturista'].'-ODS.xlsx"');
	header('Cache-Control: max-age=0');
